...
int main(int argc, char * argv[])
{
	...
	print_results("CG", Class, 14000, 0, 0, 15, t, mflops, "          floating point", verified, "3.3.1", "30 Apr 2023", "gcc", "$(CC)", "-lm", "-I../common", "-g -Wall -O3 -fopenmp -mcmodel=large", "-O3 -fopenmp -mcmodel=large", "randdp");
	if (timeron)
	{
		tmax=timer_read(1);
		if (tmax==0.0)
...
static void makea(int n, int nz, double a[], int colidx[], int rowstr[], int firstrow, int lastrow, int firstcol, int lastcol, int arow[], int acol[][(11+1)], double aelt[][(11+1)], int iv[])
{
	...
	sparse(a, colidx, rowstr, n, nz, 11, arow, acol, aelt, firstrow, lastrow, iv, 0.1, 20.0);
}
