...
int main()
{
	...
	print_results("EP", 'A', 28+1, 0, 0, nit, tm, Mops, "Random numbers generated", verified, "3.3.1", "01 May 2023", "gcc", "$(CC)", "-lm", "-I../common", "-g -Wall -O3 -fopenmp -mcmodel=large", "-O3 -fopenmp -mcmodel=large", "randdp");
	if (timers_enabled)
	{
		if (tm<=0.0)
		{
